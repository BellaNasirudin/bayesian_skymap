...
data = np.load(fdir + "data_haslam_new_nside%i_fwhm%ideg_nside%i_mask.npz" %(nside, beam_deg, nside_new))
...
def gibbs_sampling(data1, data2, covN1, covN2, covG, ntimes, operator, freqs, beta, true_s, ref_freq, indx_ref, beam_fwhm,
 nstart=0, file_s0 = None, true_g = 0, nside=128, nside_new=16, lmax_gain=10, lmax_beam = 150, op_alm=False, freq_beam=100, pixel_mask=None, debug=False):
    ...
    for ii in range(ntimes):
        ...
        rhs_eqn_g = g_rhs_vec(s0, data2, covN2, covG, operator, op_alm, mu = true_g, nside=nside, lmax = lmax_gain)
        ...
        if debug == True:
            if op_alm == False:
                lhs_val = g_lhs_matrix(true_g, s0, covN2, covG, operator)
            else:
                lhs_val = g_lhs_healpy(true_g, s0, covN2, covG, nside, len_alms, lmax = lmax_gain)
        ...
        time_start_it = time.perf_counter()
        g, info = linalg.gmres(lhs_matrix_g, rhs_eqn_g, maxiter=int(1e3))#, rtol=1e-9, atol=1e-1)#, x0 = np.random.normal(0, 0.05, len(true_g))) # 
        print("max diff of g: ", np.max(abs(true_g - g)), info)
        ...
        if info>0:
            ...
            rhs_eqn_g = g_rhs_vec(s0, data2, covN2, covG, operator, op_alm, mu = true_g, nside=nside, lmax = lmax_gain)
            g, info = linalg.cgs(lhs_matrix_g, rhs_eqn_g, maxiter=int(1e3), rtol=1e-9)#, x0 = np.random.normal(0, 0.05, np.shape(true_g)))
            time_end = time.perf_counter()
            print(g, info)
        ...
        if debug == True:
            ...
            if op_alm == False:
                true_lhs = amps_lhs(true_s, operator @ true_g , all_covN, covA, freqs, ref_freq, beta, beam_vec,
                 nside=nside, nside_new=nside_new, Nfreqs=len(freqs), beam_fwhm=beam_fwhm, lmax = lmax_beam, freq_beam = freq_beam, pixel_mask=pixel_mask)
            else:
                true_lhs = amps_lhs(true_s, 1 + hp.alm2map(true_g[:int(len_alms/2)] + 1j * true_g[int(len_alms/2):], nside, lmax=lmax_gain), all_covN, covA, freqs, ref_freq, beta, beam_vec,
                 nside=nside, nside_new=nside_new, Nfreqs=len(freqs), beam_fwhm=beam_fwhm, lmax = lmax_beam, freq_beam = freq_beam, pixel_mask=pixel_mask)
        ...
        if ((ii>1) and (ii % 250 == 0)):
            ...
            if sig2 == sig3:
                np.savez(fout + "output_lnpost_a_s_partial_%i_nstart%i_new_alm%s-%i_fwhm%ideg_sig1%i_sig2%i_nside%i_mask" %(ntimes, nstart, op_alm, ii, beam_deg, sig1, sig2, nside_new),
                ...
                np.savez(fout + "output_lnpost_a_s_partial_%i_nstart%i_new_alm%s-%i_fwhm%ideg_sig1%i_sig2%i_sig3%i_nside%i_mask" %(ntimes, nstart, op_alm, ii, beam_deg, sig1, sig2, sig3, nside_new),
                 ln_post = ln_post[:ii], all_s= all_s[:ii], all_g = all_g[:ii], chisq=chisq[:ii],all_model_global = all_model_global[:ii], all_model_haslam = all_model_haslam[:ii])
...
if __name__ == '__main__':
    ln_post, all_s, all_g, chisq, all_model_global, all_model_haslam, covG, covA = gibbs_sampling(data1, data2, covN1, covN2, covG, ntimes,
                                                        ...
                                                        true_g = gain.flatten() + 1, op_alm = op_alm, nside = nside, nside_new = nside_new,
                                                        lmax_gain =lmax_gain, lmax_beam = lmax_beam, freq_beam=freq_beam, pixel_mask=pixel_mask)

    all_data = [data1, data2]
    if sig2 == sig3:
        np.savez(fout + "output_lnpost_a_s_all_%i_nstart%i_new_alm%s_fwhm%ideg_sig1%i_sig2%i_nside%i_mask" %(ntimes, nstart, op_alm, beam_deg, sig1, sig2, nside_new),
        ...
        np.savez(fout + "output_lnpost_a_s_all_%i_nstart%i_new_alm%s_fwhm%ideg_sig1%i_sig2%i_sig3%i_nside%i_mask" %(ntimes, nstart, op_alm, beam_deg, sig1, sig2, sig3, nside_new),
         ln_post = ln_post, all_s= all_s, all_g = all_g, chisq=chisq, data1 = data1, data2 = data2, all_model_global = all_model_global, all_model_haslam = all_model_haslam, covG= covG, covA=covA)
