...
print(op_alm, ntimes, nstart, file_s0, beam_deg, flush=True)
...
data = np.load(fdir + "data_haslam_new_nside%i_fwhm%ideg_nside%i_spectral.npz" %(nside, beam_deg, nside_new))
...
print("LMAX BEAM IS ", lmax_beam, "USING %i FREQUENCY BINS" %len(freqs))
...
def gibbs_sampling(data1, data2, covN1, covN2, covG, ntimes, operator, operator_spectral, freqs, beta, true_s, ref_freq, indx_ref, beam_fwhm,
    nstart=0, file_s0 = None, mu_g = 0, nside=128, nside_new=16, lmax_gain=10, lmax_beam = 150, op_alm=False, freq_beam=100, nwalkers=20, nsample=25):
    ...
    covA = (0.1 * true_s)**2 # np.var(all_data, axis=0) #  #  0.1 # (0.1 * all_data)**2 # np.ones(len(true_s)) * 0.1**2 #
    covS = covA
    ...
    for ii in range(ntimes):
        ...
        time_start_it = time.perf_counter()
        g, info = linalg.gmres(lhs_matrix_g, rhs_eqn_g, maxiter=int(1e3))
        print(g, info)
        ...
        if info>0:
            ...
            rhs_eqn_g = g_rhs_vec(s0, data2, covN2, covG, operator, op_alm, mu = mu_g, nside=nside, lmax = lmax_gain)
            g, info = linalg.cgs(lhs_matrix_g, rhs_eqn_g, maxiter=int(1e3), rtol=1e-9)
            time_end = time.perf_counter()
            print(g, info)
        ...
        time_start = time.perf_counter()
        amps, info = linalg.bicgstab(A = lhs_matrix, b = rhs_eqn, maxiter=int(1e3))
        time_end = time.perf_counter()
        ...
        if info>0:
            ...
            rhs_eqn = amps_rhs(all_data, gain, all_covN, covA, freqs, ref_freq, beta, beam_vec, nside=nside, nside_new=nside_new, Nfreqs=len(freqs), lmax = lmax_beam, freq_beam = freq_beam)
            amps, info = linalg.cgs(A = lhs_matrix, b = rhs_eqn, maxiter=int(1e3), rtol=1e-9)
            time_end = time.perf_counter()
        ...
        sampler = emcee.EnsembleSampler(nwalkers, ndim, log_prob_haslam, 
            args=[all_data, precomp_mtrx, freqs, ref_freq, operator_spectral, all_covN, amps, gain, beam_fwhm, lmax_beam, freq_beam, nside, nside_new, -7])
        ...
        res = least_squares(log_prob_haslam, x0, args=(all_data, precomp_mtrx, freqs, ref_freq,  
         operator_spectral, all_covN, amps, gain, beam_fwhm, lmax_beam, freq_beam, nside, nside_new,  -3), ftol=5e-2, xtol=None, gtol=None, max_nfev=5e3), #method='TNC', tol=1e-12, options = {"xtol":1e-4, "ftol":1e-6, "gtol":1e9 }, #"maxiter": 3e3, 
        ...
        np.save("mcmc_samples_prior", samples)
        np.save("mcmc_lnprob_noprior_start", log_prob_haslam(samples[0,-1], all_data, precomp_mtrx, freqs, ref_freq, operator_spectral, all_covN, amps, gain, beam_fwhm, lmax_beam))
        np.save("mcmc_lnprob_noprior_final", log_prob_haslam(samples[-1,-1], all_data, precomp_mtrx, freqs, ref_freq, operator_spectral, all_covN, amps, gain, beam_fwhm, lmax_beam))
        ...
        if ((ii>1) and (ii % 100 == 0)):
            ...
            if sig2 == sig3:
                np.savez(fout + "output_lnpost_a_s_partial_%i_nstart%i_new_alm%s-%i_fwhm%ideg_sig1%i_sig2%i_spectralBeta_prior" %(ntimes, nstart, op_alm, ii, beam_deg, sig1, sig2), 
                    ln_post = ln_post[:ii], all_s= all_s[:ii], all_g = all_g[:ii], chisq=chisq[:ii], all_data= all_data, all_model = all_model[:ii], all_beta = all_beta[:ii],
                    all_model_global = all_model_global[:ii], all_model_haslam = all_model_haslam[:ii])
            else:
                np.savez(fout + "output_lnpost_a_s_partial_%i_nstart%i_new_alm%s-%i_fwhm%ideg_sig1%i_sig2%i_sig3%i_spectralBeta_prior" %(ntimes, nstart, op_alm, ii, beam_deg, sig1, sig2, sig3), 
                    ln_post = ln_post[:ii], all_s= all_s[:ii], all_g = all_g[:ii], chisq=chisq[:ii], all_data= all_data, all_model = all_model[:ii], all_beta = all_beta[:ii],
                    all_model_global = all_model_global[:ii], all_model_haslam = all_model_haslam[:ii])

    return ln_post, all_s, all_g, all_beta, chisq, all_model_haslam, all_model_global, covG, covA

if __name__ == '__main__':
    ln_post, all_s, all_g, all_beta, chisq, all_model_haslam, all_model_global, covG, covA = gibbs_sampling(data1, data2, covN1, covN2, covG, ntimes,
                                                         operator, op_spect, freqs, beta, true_s, ref_freq, indx_ref,
                                                        beam_fwhm, nstart = nstart, file_s0 = file_s0,
                                                        mu_g = gain.flatten(), op_alm = op_alm, nside = nside,
                                                        lmax_gain =lmax_gain, lmax_beam = lmax_beam, freq_beam=freq_beam)

    
    if sig2 == sig3:
        np.savez(fout + "output_lnpost_a_s_all_%i_nstart%i_new_alm%s_fwhm%ideg_sig1%i_sig2%i_spectralBeta_noprior" %(ntimes, nstart, op_alm, beam_deg, sig1, sig2), 
        ...
        np.savez(fout + "output_lnpost_a_s_all_%i_nstart%i_new_alm%s_fwhm%ideg_sig1%i_sig2%i_sig3%i" %(ntimes, nstart, op_alm, beam_deg, sig1, sig2, sig3), 
